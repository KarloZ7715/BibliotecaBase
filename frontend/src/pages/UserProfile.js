...
import { Button, Form, Alert, Spinner, Pagination } from "react-bootstrap";
...
const UserProfile = () => {
  ...
  const handleChange = (e) => {
    ...
    switch (name) {
      case "correo":
        tempValidations.correo = validateEmail(value)
          ? null
          : "El correo electrónico no tiene un formato válido.";
        break;
      case "telefono":
        tempValidations.telefono = validatePhone(value)
          ? null
          : "El número de teléfono solo debe contener dígitos.";
        break;
      case "contraseña":
        tempValidations.contraseña =
  ...
  const handleActualizar = async (e) => {
    ...
    if (!validateEmail(correo)) {
      tempValidations.correo =
        "El correo electrónico no tiene un formato válido.";
      valid = false;
    }

    if (telefono && !validatePhone(telefono)) {
      tempValidations.telefono =
        "El número de teléfono solo debe contener dígitos.";
      valid = false;
    }
  ...
  const handleEliminarTarjeta = async (id_tarjeta) => {
    if (window.confirm("¿Estás seguro de que deseas eliminar esta tarjeta?")) {
      try {
        await axios.delete(`http://localhost:8000/tarjetas/${id_tarjeta}`, {
          withCredentials: true,
        });
        setTarjetas(
          tarjetas.filter((tarjeta) => tarjeta.id_tarjeta !== id_tarjeta)
        );
              ...
              {tarjetas.map((tarjeta) => (
                <div key={tarjeta.id_tarjeta} className="tarjeta-item">
                  ...
                  <Button
                    variant="danger"
                    size="sm"
                    onClick={() => handleEliminarTarjeta(tarjeta.id_tarjeta)}
                  >
          ...
          <Form onSubmit={handleActualizar} className="update-form">
            ...
            <Form.Group className="mb-3" controlId="formContraseña">
              ...
              <Form.Text className="text-muted">
                Si deseas cambiar tu contraseña, ingresa una nueva. Debe tener
