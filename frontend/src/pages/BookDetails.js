...
function BookDetails() {
    ...
    useEffect(() => {
        const fetchBookDetails = async () => {
            try {
                const bookResponse = await axios.get(`http://localhost:8000/libros/${id}`);
                setBook(bookResponse.data);

                const valoracionesResponse = await axios.get(`http://localhost:8000/valoraciones/libro/${id}`);
                setValoraciones(valoracionesResponse.data);

                if (valoracionesResponse.data.length > 0) {
                    const suma = valoracionesResponse.data.reduce((acc, val) => acc + val.valoracion, 0);
                ...
                if (user) {
                    const pedidosResponse = await axios.get('http://localhost:8000/pedidos/', { withCredentials: true });

                    const haComprado = pedidosResponse.data.some(pedido =>
                        pedido.detallepedido?.some(detalle => Number(detalle.id_libro) === Number(id)) || false
                    );
    ...
    const submitRating = async () => {
        ...
        try {
            ...
            const suma = (isEditing ? valoraciones.reduce((acc, val) => acc + val.valoracion, 0) - userValoracion.valoracion : valoraciones.reduce((acc, val) => acc + val.valoracion, 0)) + ratingValue;
    ...
    const deleteRating = async () => {
        ...
        try {
            ...
            const suma = valoraciones.reduce((acc, val) => acc + val.valoracion, 0) - userValoracion.valoracion;
            ...
            <Container className="book-details-page mt-4">
                <Row>
                        ...
                        <div className="valoraciones image-valoraciones mt-4">
                            ...
                            <div className="d-flex align-items-center mb-2">
                                <ReactStars
                                    ...
                                    edit={false}
                                />
                                <span className="ms-2">{promedioValoracion.toFixed(2)} ({valoraciones.length} valoraciones)</span>
                            </div>
                            ...
                            {valoraciones.length > 0 ? (
                                valoraciones.slice(0, 3).map(val => (
                                    <div key={val.id_valoracion} className="mb-3">
                                        ...
                                        <small className="text-muted">{new Date(val.fecha_valoracion).toLocaleDateString()}</small>
                                    </div>
                                ))
                            ) : (
                            ...
                            {valoraciones.length > 3 && (
                                <Button variant="link" onClick={() => navigate(`/libros/${id}/valoraciones`)}>
                    ...
                    <Col md={8}>
                        ...
                        <p><strong>Descripción:</strong> {book.descripcion || 'No descripción disponible'}</p>
                        <div className="book-details">
                            <p><strong>Categoría:</strong> {book.categoria ? book.categoria.nombre_categoria : 'Sin categoría'}</p>
                            <p className="price"><strong>Precio:</strong> ${book.precio.toFixed(2)}</p>
                            <p className="stock">
                                <strong>Stock:</strong>{' '}
                                {book.stock > 0 ? (
                ...
                <Modal show={showRatingModal} onHide={closeRatingModal} centered>
                    <Modal.Header closeButton>
                        <Modal.Title>{isEditing ? 'Editar Valoración' : 'Agregar Valoración'}</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Form>
